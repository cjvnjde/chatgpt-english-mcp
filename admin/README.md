...
Review and presentation tables remain immutable, as required by the existing database triggers. Cache, scheduler, and system tables are inspectable but not directly writable. Vocabulary writes use the existing service so validation, sense identity, usefulness inference, and card creation/deletion stay consistent. Terms and selected dictionary definitions are fixed after creation, while saved context remains editable. For a context-only item, changing context updates its meaning identity and is rejected if another item for the same term already has that identity. Create a new item when a second meaning is needed. The database stores review comments and ratings, not answer transcripts. Deleting a word removes its cards; historical records are retained and may no longer have a resolvable word label.
...
An existing item's **Audio and images** section reads content-addressed Cambridge media from this server instead of contacting Cambridge again. Repeated dictionary entries that reference the same stored pronunciation render one audio control, with shared UK/US labels combined. Authenticated media is fetched into same-origin blob URLs, and the production content-security policy explicitly permits those audio and image blobs. The section also accepts up to 12 additional JPEG, PNG, GIF, WebP, or AVIF images of at most 10 MiB each. An optional example or memory cue and the original filename are stored with each attachment. Attachments belong only to that owner and vocabulary item; deleting an attachment or item removes an otherwise unreferenced custom media object.

Uploads use authenticated `POST /admin/api/vocabulary/{id}/images` with multipart fields `image`, optional `example`, and `expectedRevision`. Removal uses authenticated `DELETE /admin/api/vocabulary/{id}/images/{attachmentID}` with a JSON `expectedRevision`. Both return the updated vocabulary item and advance its revision; a stale revision returns 409 without attaching or deleting anything. Authenticated `GET /admin/api/media/{mediaId}` streams validated image/audio bytes with `no-store`, same-origin, and no-sniff response headers. The token remains in the `Authorization` header, never a media URL. The shipped nginx proxy permits 10 MiB plus 64 KiB multipart overhead only on the upload route; outer proxies must allow at least that much too. Other API routes retain the 1 MiB limit.
...
A click outside a clean editor dialog closes it. Dirty drafts require explicit confirmation before closing by the backdrop, changing views, opening review history, or signing out. Reloading or leaving the page requests the browser's unsaved-changes confirmation. Drafts are kept in memory, not persisted across a confirmed reload.
...
Admin vocabulary items include a `revision`. Metadata `PATCH`, image upload, and image removal all require the loaded positive `expectedRevision`; stale values return 409 so media changes cannot silently overwrite another writer. `PATCH /admin/api/vocabulary/{id}` returns 428 for a missing precondition and 400 for an invalid one. MCP clients do not use this admin precondition, but their metadata edits still advance the revision. Migration `018` includes context-only and context-derived identity changes, preventing stale drafts from overwriting another writer's meaning correction.
