...
`settings.js` validates and autosaves local connections and per-mode controls. Existing `contextMode` configures Deep; missing `quickContextMode` inherits it on upgrade, preserving None preferences. Each request snapshots its model, prompt, effort, and settings. `thinkingLevel` and `quickThinkingLevel` independently select `reasoning_effort`; provider default omits the parameter. Explicit values are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`, subject to provider/model support. Unsupported levels fail visibly without silent retries. Exact, unmodified legacy bundled prompts migrate to the current defaults by SHA-256 fingerprint; edited prompts are preserved.
...
The sidebar-only `SELECTION_CLEAR` command checks the conversation ID, invalidates outstanding selection captures, cancels Quick/Deep, removes the source and pending Deep work, and publishes a new blank conversation with a monotonically increasing revision. The clear button also removes the old draft and focuses manual input. Late replies and save decisions cannot restore the cleared conversation or start a new write; an MCP write already dispatched is not reversible. Other Firefox windows and saved vocabulary remain unchanged.
...
Selection starts with all due and FSRS-new cards and applies the global adaptive last-three-events/less-than-30-minute cooldown before choosing a pool. When the cooldown would force a rotation, older eligible cards can rejoin the lottery, preserving fresh alternatives and avoiding immediate repeats when alternatives exist. Future cards still identify the latest active presentation but never become eligible through relaxation.

After cooldown, any selectable due FSRS Learning (1)/Relearning (3) cards form the priority pool; they pause new introductions and mature reviews only while selectable. Otherwise, when both pools remain, sum their per-card exposure priorities and give New the resulting proportion clamped to 20%–80%; Review receives the remainder. A sole pool wins. Presented-card exposure priority recovers from 0.25 to 1 over 24 hours, grows to 2 by 30 days, and never-presented cards receive 4. This makes a large unseen backlog dominate the mix without starving due reviews. New-card weight is exposure × usefulness (low ×0.5, normal ×1, high ×2); mature-review weight is urgency × failure × exposure; learning/relearning weight is urgency × failure. Learning steps do not use soft exposure. Due urgency is `1 + min(max(overdueHours, 0) / intervalHours, 4)`, with `intervalHours = 1/6` for learning/relearning and `max(scheduledDays * 24, 24)` for mature reviews. Failure multipliers remain bounded.

Additionally, personal interest multiplies every selectable new/step/mature weight by 0.5/1/2 for low/normal/high. It does not affect cooldown eligibility, adaptive pool shares, FSRS scheduling, or deterministic future rotation. The admin likelihood calculation uses the same selection plan and weights.
...
Repair editable metadata, including context, explicitly through `vocabulary_update` or the admin editor, then retry the snapshot. The term and any selected dictionary definition remain immutable. For an item without a selected dictionary definition, changing context updates its context-derived identity and is rejected rather than merged if another saved meaning for the term already uses it. Preserve the original export/history and deliberately create a replacement before deleting an item whose immutable term or selected definition is invalid. Archiving alone does not repair an export because archived items are exported too. These fixes never rewrite immutable history automatically.
...
Snapshot schema version `4` adds vocabulary-image metadata and optional content-addressed IDs for copied dictionary media while retaining required usefulness and personal-interest metadata. Media bytes are not embedded in the Anki payload, and the Python worker validates but does not render image metadata or local IDs into Anki fields/tags. Deploy the Go exporter and Python worker together; older snapshots are rejected instead of silently weakening validation. Existing source IDs, note mappings, and Anki cards remain unchanged. Reinforcement practice state is not exported.
