...
`settings.js` validates and autosaves local connections and per-mode controls. Existing `contextMode` configures Deep; missing `quickContextMode` inherits it on upgrade, preserving None preferences. Each request snapshots its model, prompt, effort, and settings. `thinkingLevel` and `quickThinkingLevel` independently select `reasoning_effort`; provider default omits the parameter. Explicit values are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`, subject to provider/model support. Unsupported levels fail visibly without silent retries.

`prompt.js` provides the complete Deep and Quick defaults, built from the local tutor instructions and surface rules. Settings expose both prompts for multiline editing or independent reset (32,000 characters each); blank is allowed. A separately appended, read-only host guard treats references as untrusted and keeps tool availability/write consent outside editable teaching instructions. The initial user request remains exactly `What does 'selected word' mean?`, hidden in the UI.

`api.js` handles OpenAI-compatible JSON/SSE; `mcp.js` handles Streamable HTTP initialization, negotiated protocol/session headers, JSON/SSE tool results, and errors without replaying writes. Deep explanations use `dictionary_lookup`. On explicit Save, a separate Deep-model request uses fixed JSON sense-selection instructions and the chosen Deep effort; editable teaching prose cannot replace that contract. The host validates the index and calls `vocabulary_save` only if the conversation is still current. Invalid choices never write. No MCP tools are exposed to the model.

`render.js` creates DOM nodes rather than evaluating model HTML. Links allow HTTP(S); images are always enabled but require exact, dictionary-provided HTTPS URLs. Native system colors follow light/dark preferences. The sidebar preserves input during streaming, hides only the initial user message, and uses icon-only clear/save/settings controls without a meaning selector.
...
Focused regressions cover per-mode privacy migration, prompt persistence/validation, provider-default effort omission, unsupported-effort errors, context recapture/source identity, streaming before completion, stale chunks/captures, truncated-stream partial delivery, request setting snapshots, explicit-save consent, and existing transport boundaries. Actual Firefox smoke verification uses a controlled local SSE provider to hold the final chunk, inspect each mode's request, interrupt/retry Quick, and exercise native-sidebar transitions with opposite context sizes. Verify prompt edits/resets survive reload and Quick remains MCP-free, including when it shares Deep's model. Real saving verification uses a **disposable** local MCP database; never use production learning data for smoke writes. Also check native light/dark colors, narrow widths, source images, and follow-up input.
