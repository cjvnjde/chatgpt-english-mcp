...
The server implements eleven MCP tools over stateless Streamable HTTP. Inputs use strict JSON schemas and reject unknown fields. IDs, cursors, and review tokens are opaque. Timestamps are generated by the server as ISO 8601 UTC strings.
...
This removes the vocabulary item and its learning card. It does not remove cached dictionary snapshots, review attempts, or presentation events.
...
Use `learning_review_update`, not a conflicting `learning_review` retry, to correct an accepted answer. After correction, the saved rating, comment, and result are the corrected values; retries with the abandoned grade are rejected.
...
// Input
{
  reviewToken: string; // original token submitted to learning_review
  rating: ReviewRating;
  comment?: string; // omit to preserve; empty string clears
}

// Output: same shape as learning_review
```

For example, to change the latest answer from `good` to `again`:

```json
{
  "reviewToken": "<original review token>",
  "rating": "again"
}
```
