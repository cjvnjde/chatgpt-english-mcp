...
Select text and click the small dictionary icon. With the sidebar closed, only the quick model runs, without MCP. With it open, only the deep explanation runs. **Alt+Shift+E toggles the sidebar**: when closed, it opens and explains the pending selection; when open, it closes. The toolbar and **right-click → Explain in sidebar** remain open-only. Opening the sidebar dismisses the quick popup. Closing it cancels an unfinished deep response.
